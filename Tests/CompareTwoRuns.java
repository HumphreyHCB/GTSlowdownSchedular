...
public class CompareTwoRuns {
    public static void compare(String NormalRunPath, String SlowdownRunPath, List<String> methodsToCheck,
            String logFilePath) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(logFilePath))) {
            ...
            for (String method : methodsToCheck) {
                ...
                Map<Integer, Double> RDTSCNormalRunBlocks = VTuneAnalyzer
                        .getBuboProbeCpuTimes(NormalRunPath, method);
                Map<Integer, Double> RDTSCSlowdownBlocks = VTuneAnalyzer
                        .getBuboProbeCpuTimes(SlowdownRunPath, method);
                ...
                for (Integer blockId : allBlockIds) {
                    ...
                    writer.write("Method: " + method + ", Block ID: " + blockId + ", Normal Time: " + normalTime
                            + ", Slowdown Time: " + slowdownTime + ", Percentage Increase: " + percentageIncrease
                            + "%\n");
                }
                ...
                if (!percentageIncreases.isEmpty()) {
                    ...
                    if (percentageIncreases.size() % 2 == 0) {
                        medianMethodPercentageIncrease = (percentageIncreases.get(middle - 1)
                                + percentageIncreases.get(middle)) / 2.0;
                    } else {
                ...
                writer.write("Method: " + method + ", Median Percentage Increase: " + medianMethodPercentageIncrease
                        + "%\n");
                if (RDTSCNormalRunBlocks.isEmpty()) {

                } else {
                    writer.write("\n ExtraBuboData:");
                    for (String string : methodsToCheck) {
                        writer.write("\nExtraBuboData:\n");

                        // Union of block IDs that had an rdtsc section in either run
                        Set<Integer> rdtscBlockIds = new HashSet<>(RDTSCNormalRunBlocks.keySet());
                        rdtscBlockIds.addAll(RDTSCSlowdownBlocks.keySet());

                        for (Integer blockId : rdtscBlockIds) {
                            double normalTime = RDTSCNormalRunBlocks.getOrDefault(blockId, 0.0);
                            double slowdownTime = RDTSCSlowdownBlocks.getOrDefault(blockId, 0.0);

                            double percentageIncrease = 0.0;
                            if (normalTime > 0.0) {
                                percentageIncrease = ((slowdownTime - normalTime) / normalTime) * 100.0;
                            }

                            writer.write(
                                    "Method: " + method +
                                            ", Block ID: " + blockId +
                                            ", RDTSC Normal Time: " + normalTime +
                                            ", RDTSC Slowdown Time: " + slowdownTime +
                                            ", RDTSC Percentage Increase: " + percentageIncrease + "%\n");
                        }
                        writer.write("\n");
                    }
                }

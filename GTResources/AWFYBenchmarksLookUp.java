        ...
        try {
            ...
            if (jsonObject.getJSONObject("AWFY Benchmarks").has(benchmarkName)) {
                return jsonObject.getJSONObject("AWFY Benchmarks")
                        .getJSONObject(benchmarkName).getBoolean("SingleFile");
            } else {
                System.out.println("Benchmark not found: " + benchmarkName);
                return false; // Return a default/error value if the benchmark is not found
            }

        } catch (IOException e) {
            e.printStackTrace();
            return false; // Return a default/error value if file reading fails
        }
    }
